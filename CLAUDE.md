...
- Integration tests in `tests/integration/` require a live n8n server — expected to fail locally and in CI
- Unit tests: 139 files, 4585+ tests (35 skipped)
...
- `dist/` is gitignored — build artifacts don't get staged

### CI/CD Configuration
- All CI workflows use **Node 22** and `npm install` (not `npm ci`)
- Local dev uses Node 25.6.1 — lock file generated by npm 11 is not compatible with `npm ci` on Node 22
- Vitest uses `pool: 'forks'` (not `'threads'`) to avoid Node 25.x native addon crashes with better-sqlite3
- sql.js does not support FTS5 — server and test utils skip FTS5 DDL automatically via `checkFTS5Support()`
